...
| laverton              | pink   | part of the werribee line, but terminates at laverton instead of going to werribee.                                                                                  | while it is mentioned as a line in the network map, its existence is poorly documented mainly due to the fact that it is basically a service along the werribee line. it only exists to help commuters easily differentiate between services before 19:00, since services ending at werribee skip stations while services running to laverton make all stops. |
### airport rail
