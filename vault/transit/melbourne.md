...
the other two groups can be further split into two subgroups, where lines in one subgroup run through to lines in the other subgroup. here is some information about each line, its subgroups, and how the subgroups run through the city centre, with special lines italicised:

| group                                                                               | colour     | subgroup 1                                                    | subgroup 2           | via                                         |
| ----------------------------------------------------------------------------------- | ---------- | ------------------------------------------------------------- | -------------------- | ------------------------------------------- |
| <span class="box" style="background-color: #f178af; color: black">cross-city</span> | pink       | werribee, williamstown, *laverton*                            | sandringham          | flinders street and southern cross stations |
...
| flemington racecourse | grey   | runs clockwise from flinders street station to southern cross and north melbourne stations, then further northwest to showgrounds and flemington racecourse stations | it is also referred to as the special event line and used only for special events happening at or near the racecourse, including races and university examinations. unlike the stony point line, it does not share a colour wit                                                                                                                                           |
| laverton              | pink   | part of the werribee line, stopping at laverton                                                                                                        while it is mentioned as a line in the network map, its existence is poorly documented mainly due to the fact that it is basically a shorter version of the werribee line. it only exists to help commuters easily differentiate between services before 19:00, since services ending at werribee skipped stations while services running to laverton made all stops. weribee   weribee   |
### airport rail
